...
void QueriesExecutor::executeAllQueries(EventStorage& events, ClientsStorage& clients, PackageStorage& packages){
    ...
    for(int j=0;j<this->numQueries;++j){
        ...
        }else if(this->queries[j].type == "CA"){
...
void QueriesExecutor::executeWarehouseQueriesAtTimeInterval(Query query, EventStorage& events){
    ...
    std::cout<< std::setfill('0') << std::setw(6) << query.time<<" CA "
    ...
    std::cout<<"Movimentacoes armazem "<<
    std::setfill('0') << std::setw(3) << query.warehouseId<<std::endl;
    while( i < events.getCurrentSize() && events.getEvent(i).time <= query.finalTime){
        Event e = events.getEvent(i);
        if(e.eventType == "AR" && e.warehouseDestination == query.warehouseId){
            std::cout<<"Pacote "<< std::setfill('0') << std::setw(3) << e.packageId<<" armazenado no armazem "
            <<std::setfill('0') << std::setw(3) << e.warehouseDestination<< " no tempo "
            <<std::setfill('0') << std::setw(3) << e.time<<std::endl;
        }else if(e.eventType == "RM" && e.warehouseDestination == query.warehouseId){
            std::cout<<"Pacote "<< std::setfill('0') << std::setw(3) << e.packageId<<" removido do armazem "
            <<std::setfill('0') << std::setw(3) << e.warehouseDestination << " no tempo "
            <<std::setfill('0') << std::setw(3) << e.time<<std::endl;
        }else if(e.eventType == "UR" && e.warehouseDestination == query.warehouseId){
            std::cout<<"Pacote "<< std::setfill('0') << std::setw(3) << e.packageId<<" rearmazenado no armazem "
            <<std::setfill('0') << std::setw(3) << e.warehouseDestination <<" no tempo "
             <<std::setfill('0') << std::setw(3) << e.time<<std::endl;
        }else if(e.eventType == "EN" && e.warehouseDestination == query.warehouseId){
            std::cout<<"Pacote "<< std::setfill('0') << std::setw(3) << e.packageId<<" entregue no armazem "
            <<std::setfill('0') << std::setw(3) << e.warehouseDestination << " no tempo "
            <<std::setfill('0') << std::setw(3) << e.time<<std::endl;
        }
        i++;
