...
void UniversalSorter::universal_sorter(int *v, int v_size, int partition_threshold, int breaks_threshold)
{
...
void UniversalSorter::register_partition_statistics(PartitionStatistics *statistics, int num_partitions, int partition_size, int statistics_size)
{
  if(num_partitions < 0 || num_partitions > statistics_size ){
...
int UniversalSorter::determine_partition_threshold(int *v, int v_size, double cost_threshold)
{
  ...
  *v_copy = new int[v_size],
  ...
  if (!v_copy) {
    std::cerr << "Error alocating memory.\n";
    return -1;
  }
  ...
  while (difference_max_min_cost > cost_threshold && num_partitions >= 5)
  {
    ...
    for (i = min_partition_size_range; i <= max_partition_size_range; i += step)
    {
      ...
      this->register_partition_statistics(statistics, current_index, i, num_partitions);
...
void UniversalSorter::register_break_statistics(BreaksStatistics *statistics_quick_sort, int num_breaks, int quant_num_breaks_tested, int current_index)
{
  if(current_index < 0 || current_index > quant_num_breaks_tested){
...
//prints the final result with the number of breaks whose cost is similiar to quicksort cost
...
//determine the number of break which is better to use insertion sort than quicksort
void UniversalSorter::determine_break_threshold(int partition_threshold, int *vet, int vet_size, double cost_threshold, int seed)
{
  int min_num_breaks_range = 1, max_num_breaks_range = vet_size / 2,
  step = (int)(max_num_breaks_range - min_num_breaks_range) / 5, 
  quant_num_breaks_tested = 5, iter = 0, i = 0, min_cost_index = 0, current_index = 0;
  ...
  while (difference_max_min_cost > cost_threshold && quant_num_breaks_tested >= 5)
  {
    quant_num_breaks_tested = this->find_index_by_partition(max_num_breaks_range, min_num_breaks_range, step) + 1;

    std::cout<<std::endl;
    current_index = 0;
    std::cout << "iter "<<iter << std::endl;

    statistics_insertion_sort = new BreaksStatistics[quant_num_breaks_tested];
    statistics_quick_sort = new BreaksStatistics[quant_num_breaks_tested];

    if (!statistics_insertion_sort) {
    std::cerr << "Error alocating memory.\n";
    return;
    }
    if (!statistics_quick_sort) {
    std::cerr << "Error alocating memory.\n";
    return;
    }
    ...
    {
      //use sorted vet and create i breaks to test it with quicksort
      ...
      this->register_break_statistics(&statistics_quick_sort[current_index], i, quant_num_breaks_tested, current_index);
      this->print_statics_quick(statistics_quick_sort[current_index]);
      this->operation_counter.resetcounter();
      
      //use sorted vet and create i breaks to test it with insertionsort
      this->shuffleVector(vet, vet_size, i, seed);
      this->sorter.insertionSort(vet, 0, vet_size - 1, &this->operation_counter);
      this->register_break_statistics(&statistics_insertion_sort[current_index], i, quant_num_breaks_tested, current_index);
      this->print_statics_insertion(statistics_insertion_sort[current_index]);
      this->operation_counter.resetcounter();
      current_index++;
    }
    //finds best number of breaks tested
    min_cost_index = this->find_num_breaks_with_min_cost_difference(statistics_quick_sort, statistics_insertion_sort, quant_num_breaks_tested);
    int last_min_num_breaks_range = min_num_breaks_range;
    int last_step = step;
    this->find_new_range(min_cost_index, &min_num_breaks_range, &max_num_breaks_range, quant_num_breaks_tested, &step);
    ...
    this->print_break_thershold_result(this->find_partition_size(min_cost_index, last_min_num_breaks_range, last_step), difference_max_min_cost, quant_num_breaks_tested);
