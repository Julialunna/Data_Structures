...
class UniversalSorter{
    ...
    int count_breaks(item *vet, int size);
    void universal_sorter(item *vector, int vector_size, int partition_threshold, int breaks_threshold);
    ...
    void register_partition_statistics(PartitionStatistics *statistics, int num_partitions, int partition_size);
    ...
    int determine_partition_threshold (item *v, int v_size, double cost_threshold);
    void shuffleVector(item *vet, int vet_size, int num_breaks, int seed);
