...
void Package::calculate_route(Graph<int>& warehouses){
   this->route = warehouses.Dijkstra(
        this->warehouse_origin_id,
        this->warehouse_destination_id
    );
}

int Package::calculate_transport_time_bfs(Graph<int>& warehouses){

    List<int> bfs_route = warehouses.BreadthFirstSearch(
        this->warehouse_origin_id,
        this->warehouse_destination_id
    );
    int total = 0;
    
    if (bfs_route.getSize() <= 1)
        return 0;
    for (int i = 1; i < bfs_route.getSize(); i++) {
        int from = bfs_route.getItem(i);
        int to = bfs_route.getItem(i+1);
        List<Edge>* neighbors = warehouses.FindEdgeNeighbors(from);
        for (int j = 1; j <= neighbors->getSize(); j++) {
            Edge e = neighbors->getItem(j);
            if (e.dest == to) {
                total += e.peso;
                break;
            }
...
int Package::calculate_tranport_time(Graph<int>& warehouses){
    int total = 0;
    List<int>* r = this->get_route();
    if (r->getSize() <= 1)
        return 0; // it is already at the destination

    for (int i = 1; i < r->getSize(); i++) {
        int from = r->getItem(i);
        int to = r->getItem(i+1);
