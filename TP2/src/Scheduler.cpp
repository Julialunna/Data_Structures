...
void Scheduler::execute_transport_event(Package* packages, Warehouse* warehouses, int warehouse_origin_id, int warehouse_destination_id, int current_time){
...
void Scheduler::simulate_deliveries(int num_packages, Package* packages, int num_warehouses, Warehouse* warehouses){
    ...
    while(this->is_all_packages_delivered(num_packages, packages) == false){
        ...
        if(std::stoi(next_event.event_key.substr(12, 1)) == 1){
            ...
            this->execute_transport_event(packages, warehouses,warehouse_origin_id , warehouse_destination_id, current_time);
